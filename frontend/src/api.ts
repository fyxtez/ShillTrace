import type { Channel, Health, HistoryPoint, Shill, WalletMention } from './types'
...
export const api={
  ...
  channels:()=>request<Channel[]>('/api/channels'), wallets:()=>request<WalletMention[]>('/api/wallets'),
  // Wallet deletion targets one detected mention so repeated mentions of the same address remain independently auditable.
  removeWallet:(id:number)=>request(`/api/wallets/${id}`,{method:'DELETE'}), seen:(id:number)=>request(`/api/shills/${id}/seen`,{method:'POST'}),
