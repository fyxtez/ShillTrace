...
import { BellRing, CircleSlash2, Database, ExternalLink, Mail, Menu, Send, Users, WalletCards } from 'lucide-react'
import type { Page } from '../types'
import './Sidebar.css'

export function Sidebar({ page, setPage, unseen, walletUnseen, collapsed, setCollapsed }: { page: Page, setPage: (p: Page) => void, unseen: number, walletUnseen: number, collapsed: boolean, setCollapsed: (value: boolean) => void }) {
    // The hamburger now toggles a compact sidebar instead of presenting a dead control.
    const items: [Page, string, ReactNode][] = [['new', 'New Shills', <BellRing />], ['all', 'All Tokens', <Database />], ['wallets', 'Wallets', <WalletCards />], ['channels', 'Channels', <Users />], ['ignored', 'Ignored Channels', <CircleSlash2 />]]
