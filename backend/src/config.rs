...
impl Config {
    pub fn from_env() -> Result<Self> {
        Ok(Self {
            ...
            telegram_api_id: required("TELEGRAM_API_ID")?
                .parse()
            ...
            telegram_session_path: env::var("TELEGRAM_SESSION_PATH")
                .unwrap_or_else(|_| "signal_ledger.session".into())
                .into(),
            photos_dir: env::var("PHOTOS_DIR")
                .unwrap_or_else(|_| "storage/channel-photos".into())
                .into(),
            market_poll_seconds: env::var("MARKET_POLL_SECONDS")
                .unwrap_or_else(|_| "15".into())
                .parse()
                .context("invalid MARKET_POLL_SECONDS")?,
        })
    }
}
