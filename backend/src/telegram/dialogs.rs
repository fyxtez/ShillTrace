use grammers_client::{Client, peer::{Dialog, Peer}};
use sqlx::PgPool;
use std::{path::{Path, PathBuf}, time::Duration};

pub async fn sync_channels(client: &Client, pool: &PgPool, photos_dir: &Path) -> anyhow::Result<usize> {
    ...
    while let Some(dialog) = dialogs.next().await? {
        let Some(id) = dialog.peer.id().bare_id() else { continue };
        let kind = match &dialog.peer { Peer::Channel(_) => "channel", Peer::Group(_) => "group", Peer::User(_) => "user", _ => "other" };
        ...
        if kind == "channel" {
            ...
            let has_photo = if path.exists() {
                tracing::debug!(channel_id = id, channel = name, "Using cached channel photo");
                true
            ...
            sqlx::query("UPDATE channels SET has_photo=$2 WHERE telegram_id=$1").bind(id).bind(has_photo).execute(pool).await?;
...
async fn download_photo(client: &Client, dialog: &Dialog, path: &PathBuf) -> bool {
    let Ok(Some(photo)) = dialog.peer.photo(true).await else { return false };
